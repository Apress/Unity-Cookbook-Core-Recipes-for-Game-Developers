...
public class Health : MonoBehaviour, IHealth
{
    ...
    public event Action OnPlayerDeath;// C# Event invoked from Die() and listened for in NPCStateMachine script.
    public event Action OnNpcDeath;//Being listened for in NPCStateMachine script.
    ...
    public void TakeHealth(int amount)
    {
        if (!isDead)
        {
            ...
            if (currentHealth <= 0)
                ...
                OnHealthDepleted?.Invoke(gameObject.tag); //ACC - Event being listened for in NPCStateMachine script
        }        
    }
    public void Die()
    {
        ...
        {
                OnNpcDeath?.Invoke();//Being listened for in NPCStateMachine script
        }
        else
        {
            ...
            //Event being listened for in NPCStateMachine. Ensures that the NPC that finally killed player stops attacking and continues patrolling
